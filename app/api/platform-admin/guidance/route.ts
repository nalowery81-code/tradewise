...
export async function GET(request: Request) {
  ...
  const { data, error } = await supabaseServer.from('GuidanceLibrary')
    .select('id, created_at, updated_at, title, guidance_text, scope, topic, priority, status, source_review_id, source_flag_id, activated_at')
    .order('updated_at', { ascending: false }).limit(500)
  if (error) return jsonNoStore({ error: 'Could not load guidance library.' }, { status: 500 })
...
export async function PATCH(request: Request) {
  ...
  const { data, error } = await supabaseServer.from('GuidanceLibrary').update(updates).eq('id', id)
    .select('id, created_at, updated_at, title, guidance_text, scope, topic, priority, status, source_review_id, source_flag_id, activated_at').single()
